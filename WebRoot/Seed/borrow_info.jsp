  ...
  DbBean db=new DbBean();
   ArrayList list = new ArrayList();
				      for(int k = 0; k <borrow.size();k++){
				           if(!"".equals(borrow.get(k))){
				              if(!" ".equals(borrow.get(k))){
				              	 if(!"null".equals(borrow.get(k))){
				           	        if(borrow.get(k)!=null)list.add(borrow.get(k));
				                 }
				              }        
				           }
				        }
					...
					<%if(type.equals("admin")){%>	
			         ...
			         <%} %>				  
		               ...
		               %>
				            ...
				            { 			            	
				            %>					           
							    <td  align="center"><%=list.get(9*i+j)%></td> 
							    <%
							  
							  }%>
							   
							  <% if(type.equals("admin")){%>
							   <td align="center"><a href="borrow_update.action?id=<%=list.get(9*i)%>"><img src="/maizego/images/add.png" width="25px"/></a></td>
							   <td align="center"><a href="borrow_delete.action?id=<%=list.get(9*i) %>" onclick="return check()"><img src="/maizego/images/e.jpg" width="25px"/></a></td>
							 <%} %>  
						   </tr> 
					     <%
					       }//for 
